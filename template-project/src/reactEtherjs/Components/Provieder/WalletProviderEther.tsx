import React, { useEffect, useState } from "react";
import WalletContext from "./WalletContext";
import { ethers } from "ethers";
import { IWalletContext, IWeb3Provider } from "../../types/context";
import Web3Modal, { getInjectedProvider, getInjectedProviderName, getProviderInfoByCheck } from "web3modal";
import { useConfig } from "../../hooks/useConfig";
import WalletConnectProvider from "@walletconnect/web3-provider";
import { IConnectorProps } from "../../types/props";
import CoinbaseWalletSDK from "@coinbase/wallet-sdk";

export default function WalletProviderEther(props: React.PropsWithChildren<any>) {
  const [open, setOpen] = useState(false);
  const config = useConfig();

  const [provider, setProvider] = useState<IWalletContext>({
    provider: new ethers.providers.JsonRpcProvider(config.networks[config.config.Network].RPC_STATIC),
    address: "0x0000000000000000000000000000000000000000",
    isConnected: false,
  });
  console.log("wallet provider");
  const providerOptions = {
    walletconnect: {
      package: WalletConnectProvider,
      options: {
        appName: config.config.AppName,
        rpc: {
          [config.networks[config.config.Network].CHAIN_ID]: config.networks[config.config.Network].RPC_STATIC,
        },
      },
    },
    coinbasewallet: {
      package: CoinbaseWalletSDK,
      options: {
        appName: config.config.AppName,
        rpc: config.networks[config.config.Network].RPC_STATIC,
        chainId: config.networks[config.config.Network].CHAIN_ID,
        darkMode: false,
      },
    },
    injected: {
      display: {
        logo: "data:image/png;base64,",
        name: "Metamask",
        description: "Connect with the provider in your Browser",
      },
      package: null,
    },
  };
  const web3Modal = new Web3Modal({
    cacheProvider: true,
    providerOptions,
    disableInjectedProvider: false,
  });

  console.log();
  useEffect(() => {
    if (web3Modal.cachedProvider) {
      web3Modal.connect().then(async (provider: any) => {
        const web3Provider = new ethers.providers.Web3Provider(provider);
        provider.on("accountsChanged", onAccChange(provider));
        const signer = web3Provider.getSigner();
        setProvider({
          provider: signer,
          address: await signer.getAddress(),
          isConnected: true,
        });
      });
    }
  }, []);

  const onAccChange = (provider: any) => {
    return async (accounts: string[]) => {
      console.log("accountsChanged", accounts);
      const web3Provider = new ethers.providers.Web3Provider(provider);
      const signer = web3Provider.getSigner(accounts[0]);
      setProvider({
        provider: signer,
        address: await signer.getAddress(),
        isConnected: true,
      });
    };
  };

  const loadWeb3Modal = async () => {
    const provider = await web3Modal.connect();
    provider.on("accountsChanged", onAccChange(provider));
    const web3Provider = new ethers.providers.Web3Provider(provider);
    const signer = web3Provider.getSigner();
    setProvider({
      provider: signer,
      address: await signer.getAddress(),
      isConnected: true,
    });
    localStorage.setItem("ConnectedWallet", "true");
  };

  return (
    <React.Fragment>
      <WalletContext.Provider value={provider}>
        <button
          id="wallet_connect_phantom_button"
          key="wallet_connect_phantom_button"
          onClick={loadWeb3Modal}
          style={{ display: "none" }}
        ></button>
        <button
          id="wallet_disconnect_phantom_button"
          key="wallet_disconnect_phantom_button"
          onClick={async () => {
            setProvider({
              provider: new ethers.providers.JsonRpcProvider(config.networks[config.config.Network].RPC_STATIC),
              address: "",
              isConnected: false,
            });
            localStorage.setItem("ConnectedWallet", "");
            if (provider.provider instanceof ethers.providers.JsonRpcSigner) {
              // @ts-ignore
              if (provider.provider.disconnects) {
                // @ts-ignore
                provider.provider.disconnects();
              }
            }
          }}
          style={{ display: "none" }}
        ></button>
        {props.children}
      </WalletContext.Provider>
    </React.Fragment>
  );
}
